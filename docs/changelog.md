...
## Session 34 — 2026-04-30 — Interactive Tracemark legend (feature branch)
- **NOT MERGED TO MAIN.** Lives on `feature/tracemark-legend` for preview-deploy validation before any production rollout.
- **`components/TracemarkLegend.tsx` (new)**: client wrapper around `<Tracemark>`. Adds an absolute-positioned overlay of 9 invisible `<button>` regions, one per patch, positioned by percentage of the wrapper width/height (so the overlay stays aligned at any breakpoint). Each button responds to hover (`onMouseEnter`/`onMouseLeave`), focus (`onFocus`/`onBlur`), and click (toggle).
- **Slug → readable mappings** (local to the component, not exported from `lib/schema.ts`): MEDIUM_LABEL, SEED_LABEL, REFERENCE_LABEL + WEIGHT_LABEL, TEACHER_LABEL, HELPER_LABEL, AI_KIND_LABEL, AI_STAGE_LABEL, AI_AWARENESS_LABEL, COLLABORATOR_LABEL. `readAnswer(patchId, data)` switches per patch and produces a single human-readable string per the spec — e.g. `"Digital 2D, started as a memory that wouldn't leave you alone"` for patch1, `"artist portfolios (a lot), music (some)"` for patch3, `"A mix of directing and making"` for patch8 when value is 4–7.
- **Label panel** below the Tracemark: warm off-white `#F8F7F6` (matches the Grace box's bg, no left-border accent — informational, not a quote). Always rendered with a `min-h-[112px]` so the layout doesn't jump. `aria-live="polite"` so screen readers announce changes. When no patch is active and data is loaded, panel shows "Tap a patch to see what it maps to." in muted italic. When skeleton (no data yet), panel renders empty — the existing `Making your mark…` line below already speaks loading.
- **Active state behavior**: hover sets active, mouseleave clears. Focus sets, blur clears. Click toggles (tap-the-active-patch to clear). No outside-click auto-clear — keeps the last-active patch visible until another patch is touched, avoids janky re-clears on mobile.
- **`app/result/result-content.tsx`**: swapped `<Tracemark data={…} className="…" />` for `<TracemarkLegend data={…} className="…" />`. The download flow is unchanged — `tracemarkRef.current.querySelector('svg')` still finds the inner Tracemark SVG; the overlay is absolute-positioned over the SVG (not nested inside it), so PNG exports are clean with no overlay artifacts.
- **`docs/backlog.md`**: added a new top-level "In review (preview branch)" section pointing at `feature/tracemark-legend`. Removed the "Interactive Tracemark legend" bullet from the existing "Loading state polish (phase 2)" section.
- Build + lint clean.
...
- **`app/api/grace/route.ts` logging audit**: added `console.error('[grace] …')` lines above every non-200 return — rate limit hit (logs the IP), missing `ANTHROPIC_API_KEY`, invalid JSON body (logs parse error), missing submission in body, Anthropic API error (logs status + body slice), empty grace text (logs raw content slice), generic catch block at the end (logs message + stack). Lifted `getIP(request)` into a local `ip` so the rate-limit log can name the offender. Success paths untouched.
- **`app/api/tally-submission/route.ts` logging audit**: catch block now `console.error('[tally-submission] sid=%s failed:', sid, …)` before returning. Style mirrors the existing `[og]` and `[result/metadata]` logs.
- **`docs/backlog.md`**: removed the "Error boundary on result page" entry from the Technical debt section — now shipped.
- Build + lint clean.

## Session 29 -- 2026-04-30 -- Landing page OG image at /api/og?landing=1
- **`app/api/og/route.tsx`**: GET handler now branches on `landing=1`. The existing sid path is unchanged; the new branch skips submission fetch and renders a landing-OG layout — 1200×630, `#F5F5F5` background, top section (~180px) carrying "Creative Trace" (56px / 700 / `#37352F`) and the tagline (24px / `#666`), bottom section (~450px) carrying a horizontal row of 5 sample Tracemarks at 200px each. Indices `[0, 3, 6, 9, 12]` from the SAMPLES sequence — visually distinct medium colors and patch profiles.
- **`buildSample` machinery duplicated** into the OG file from `app/page.tsx` (the constants `SEEDS`, `MEDIUMS`, `ALL_REFERENCE_TILES`, `ALL_HELPERS`, `ALL_COLLABORATORS`, `TEACHER_SETS`, `AI_KIND_SETS`, `AI_STAGES`, `AI_AWARENESS`, `REFERENCE_PROFILES`, `HELPER_COUNTS`, `COLLAB_COUNTS`, `DIRECTIONS`, `AI_USED_FLAGS`, plus `buildReferences` / `buildHelpers` / `buildCollaborators` / `buildSample`). Mirrors the existing self-contained-OG pattern that already duplicates Tracemark constants. `SeedType` added to the schema-type imports.
- **`TracemarkSVG`** gains an optional `size?: number` prop (default 540). The SVG `width` and `height` attributes use `size`; the `viewBox` stays `0 0 540 540`, so all internal patch geometry scales down proportionally. The single-mark sid path passes no `size` — behavior unchanged.
- **Pattern fetching for the row**: each of the 5 sample marks gets its Patch 1 pattern inlined via `getPatternPaths`. Fetched in parallel via `Promise.all` before rendering JSX.
- **Satori-friendly layout**: outer column flex with explicit child heights (180 + 450 = 630). Bottom row uses `display: 'flex'` with `justifyContent: 'center'`; per-mark wrappers carry explicit `display: 'flex'` and `marginLeft: idx === 0 ? 0 : 20` to preserve the 20px gap without using the `gap` shorthand (Satori's older support is unreliable). All multi-child containers carry `display: 'flex'`.
- **`app/layout.tsx`**: root metadata now exports `openGraph` and `twitter` blocks pointing at `/api/og?landing=1`. The result page's `generateMetadata` already sets its own `openGraph.images` to `/api/og?sid=…`, which overrides the layout-level value per-route — verified the route table still shows `ƒ /api/og` and the result page still uses the per-result OG image.
- Build + lint clean.
...
- **`<img>` over `next/image`**: inline SVG illustrations don't benefit from Next's image optimization (no resize / format conversion), and `next/image` would require explicit width/height that fights the `h-full w-full` layout. Added scoped `eslint-disable-next-line @next/next/no-img-element` comments above each `<img>` to keep lint clean while preserving the explicit `<img>` element.
- Build + lint clean.

## Session 26 -- 2026-04-30 -- Prod metadataBase, grace prompt fix, Tally text normalization
- **`app/layout.tsx` metadataBase**: now resolves to `https://creativetrace.art` when `VERCEL_ENV === 'production'` instead of `https://${VERCEL_URL}`. Preview deploys still use `VERCEL_URL` (so per-branch shares preview the right deploy); local dev still uses `localhost:3000`. Production OG crawlers will now see absolute URLs against the apex domain — previously they resolved against the deployment-specific subdomain (e.g. `creative-provenance-abc123.vercel.app`), which canonical share-target validators don't love. Mirrors the override that already lived on `app/result/page.tsx` `generateMetadata`; with the root layout now correct, the result-page override could in principle be dropped, but it's left in place for explicitness — does no harm.
- **`app/api/grace/route.ts` system prompt**: `aiAwareness field` → `trainingDataAwareness field`. The schema field on `aiGenerator` is `trainingDataAwareness`, never `aiAwareness` — `aiAwareness` was the Tally-side context label used by the mapper. The wrong field name in the system prompt would have led the model to look for a key that doesn't exist on the JSON it receives.
- **`lib/tally.ts` normalize-then-lookup**: replaced the manual curly/straight quote duplicate keys with a normalize step. New `normalize(text)` lowercases + trims + replaces `'`/`'`/`` ` ``/`´` → `'`, `"`/`"` → `"`, `—`/`–` → `-`, `…` → `...`, and collapses whitespace. New `normalizedLookup(text, table)` tries exact match first (cheap), then scans table keys with `normalize` on both sides; returns `{ found, value }` so callers can distinguish "matched and intentionally maps to null" from "no match" (warn-and-skip).
- **`mapOne` and `mapMany` rewritten** on top of `normalizedLookup`. Same external behavior, same warning shape, but now resilient to whichever punctuation form Tally happens to send for a given option.
- **Cleaned up duplicate keys** in `SEED_TEXT_TO_SCHEMA`, `AI_HELPER_TEXT_TO_SCHEMA`, `AI_STAGE_TEXT_TO_SCHEMA`, `AI_AWARENESS_TEXT_TO_SCHEMA`, `COLLABORATOR_TEXT_TO_SCHEMA`. Each option now has one canonical key — the curly-quote / em-dash / curly-ellipsis form (Tally's default). Removed the straight-quote, hyphen, and `...` variants. Net `lib/tally.ts` is shorter and the mappings are easier to audit against the live form. Also collapsed `'Other...'` (straight) into `'Other…'` (curly) under the same rule.
- **Q3 reference bucket lookup** (`mapTallyToProvenance`) now falls back to a `normalize`-based scan when the direct key lookup misses. The bucket table only has 3 entries (`Barely`, `A little`, `A lot`) with no curly variants today, but the same robustness applies if Tally ever wraps the bucket labels in different punctuation.
- Build + lint clean.

## Session 25 -- 2026-04-30 -- Inline seed pattern in PNG download
- **Bug**: clicking Download Tracemark on the result page produced a PNG with Patch 1 rendered as solid medium color — the seed pattern was missing. The other 8 patches downloaded correctly. Cause is the same one we hit with the OG renderer in Session 24: the live SVG references the pattern via `<image href="/patterns/…">`, but neither Satori (OG) nor canvas `drawImage` (download) resolves external `<image>` refs. The fix is the same — fetch the pattern at runtime, regex-extract its `<path>` elements, inline them as `<path>` children of a transformed `<g>`.
- **`app/result/result-content.tsx`**: new module-level `getPatternPaths(fileName)` and `inlinePatternForDownload(cloned)` helpers. The first fetches `/patterns/pattern-${fileName}.svg`, strips `<defs>...</defs>`, regex-extracts each `<path>`'s `d` / `stroke` / `stroke-width` / `stroke-miterlimit` / `fill`, caches successful parses in a module-level `Map`. The second walks the cloned SVG, finds the `<image>` element whose `href` (or legacy `xlink:href`) contains `/patterns/`, derives the file-name segment from the href, fetches the parsed paths, and replaces the `<image>` with a `<g transform="translate(-2.11,-2.11) scale(1.02344)" shape-rendering="geometricPrecision">` carrying the inlined `<path>` elements. Created via `createElementNS('http://www.w3.org/2000/svg', …)` so the SVG namespace stays clean. Failure (network, 404, no paths matched, no `<image>` element in the cloned tree) is non-fatal — the download proceeds with Patch 1 as solid color, just like the live render does when seed is undefined.
- **`downloadTracemarkPNG`**: now `await inlinePatternForDownload(cloned)` between the clone+xmlns step and the `XMLSerializer` step. The rest of the flow (canvas, drawImage, toBlob, anchor click) is unchanged.
- **Why derive seed from the href instead of the submission**: the cloned SVG is the source of truth for what's actually on screen. If the live render decided not to draw a pattern (seed undefined / unmapped), there's no `<image>` element to replace and we skip the inline step entirely. No need to thread submission data through the download function.
- **`shape-rendering="geometricPrecision"`** on the inlined `<g>` overrides the parent SVG's `crispEdges`. Pattern paths are mostly cubic Beziers; without AA they'd download jaggy. Same fix the OG renderer ships (Session 24's anti-alias commit).
- **Cross-reference comments** added to both `app/result/result-content.tsx` and `app/api/og/route.tsx` flagging that the helpers are intentional duplicates and must stay aligned on path-extraction + oversize/offset values. If a future pattern alignment fix lands, both files update together.
- Build + lint clean.
